...
function ModelContent({
  ...
  const hotspotMeshes = useMemo(() => {
      return childPositions.map((p) => {
          ...
          const roomAssignedCode = data?.metadata.map(d => d.tenantRoom?.assigned_name === p.name ? d.tenantRoom?.assigned_code : null).filter(Boolean)[0] || null;
          ...
          .map((item) => {
            ...
            return (
              <Button 
                ...
                ${item.isBlocked ? 'opacity-50 pointer-events-none' : ''}
                `
              }>
                <IconComponent 
                  className={`
