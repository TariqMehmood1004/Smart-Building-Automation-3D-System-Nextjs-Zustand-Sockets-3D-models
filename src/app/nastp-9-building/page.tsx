...
function ModelContent({
  ...
  console.log(chalk.blue(`scene: ${JSON.stringify(scene, null, 4)}`));
...
export default function ModelViewer() {
  ...
  const [, setDrawerIndex] = useState<number | null>(null);
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [drawerDevice, setDrawerDevice] = useState<HvacMideaDevice | null>(null);

  const [, setDrawerChildName] = useState<string>("");
  ...
  const tenantRoomAssignedName = data?.metadata.map(t => t.tenantRoom?.assigned_name)
  console.log(chalk.blue(`tenantRoomAssignedName: ${tenantRoomAssignedName}`));
        ...
        <Suspense
          ...
          <ModelContent
            ...
            onOpenDrawer={(index, childName) => {
              ...
              const matchedDevice = data?.metadata.find(d => d.tenantRoom?.assigned_name === cleaned) || null;

              setDrawerDevice(matchedDevice);
              setDrawerIndex(index);
              setDrawerChildName(cleaned);
              setIsMatchedRoom(!!matchedDevice);
              onOpen();
            }}
          />
        ...
        <DrawerContent>
          {(onClose) => (
            <>
                ...
                <DrawerBody className="w-full h-full flex flex-col items-center justify-center orbitron-font">
                  <ACControlDrawer
                      data={drawerDevice ? [drawerDevice] : null}
                      onClose={onClose}
