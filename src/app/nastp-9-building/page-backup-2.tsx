...
function ModelContent({
  ...
  //   const obj = scene.getObjectByName("3D Model") as THREE.Group | undefined;
    
  //   // const topChildren = scene.children.map((c) => c.children).flat().filter(c => c.children.length > 0);
  //   const topChildren = scene.children.map((c) => c);
  //   console.log(chalk.blue(`top-children: ${JSON.stringify(topChildren, null, 4)}`));
  ...
  useLayoutEffect(() => {
      ...
      const getObjectChildren = (allObjects: THREE.Object3D[]) => {
          return allObjects.filter((obj) => obj.type === "Object3D");
      }

      const allObjectsChildren = getObjectChildren(allObjects);
       ...
       * {
            ...
            "object": {
              "uuid": "39bca1f5-6859-4f45-8866-f3f454ecbe9b",
              "type": "Group",
              "name": "Scene",
              "layers": 1,
              "matrix": [
              ...
              "children": [
                {
                  "uuid": "ef8a78d3-340e-492d-80b7-385eb2411daa",
                  "type": "Object3D",
                  "name": "MODEL",
                  "userData": {
                    "name": "MODEL"
       ...
       */
      const objs = allObjectsChildren.map((c) => c);

      console.log(
          chalk.blue(`objs: ${JSON.stringify(objs, null, 4)}`)
      );

      // console.log(
      //     chalk.blue(`all-objects: ${JSON.stringify(allObjects, null, 4)}`)
      // );

      const positions = allObjects
          .filter((child) => child.type === "Mesh" || child.children.length > 0)
          .map((child, index) => {
              child.updateMatrixWorld(true);

              let worldVec = new THREE.Vector3();

              const mesh = child.type === "Mesh"
                  ? (child as THREE.Mesh)
                  : child.getObjectByProperty("type", "Mesh");
...
export default function ModelViewer() {
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center">
      ...
      <Canvas camera={{ fov: 3, near: 7.5, far: 1000 }}>
        <Suspense
          fallback={
            <Html center>
